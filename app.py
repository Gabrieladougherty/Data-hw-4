from flask import Flask, request, jsonify
import joblib
import pandas as pd
import numpy as np

app = Flask(__name__)

model = joblib.load("model.pkl")

REQUIRED_FIELDS = [
    "delivery_days",
    "delivery_vs_estimated",
    "order_purchase_dow",
    "total_price",
    "total_freight",
    "n_items",
    "n_sellers",
    "avg_price",
    "payment_value",
    "payment_installments",
    "product_category",
    "seller_state",
    "payment_type",
]
@app.route("/health", methods=["GET"])
def health():
    return jsonify({"status": "healthy", "model": "loaded"})

def validate_record(rec):
    errors = {}

    # Required fields
    for f in REQUIRED_FIELDS:
        if f not in rec:
            errors[f] = "missing"

    # Numeric fields must be numbers
    for f in [
        "delivery_days",
        "delivery_vs_estimated",
        "order_purchase_dow",
        "total_price",
        "total_freight",
        "n_items",
        "n_sellers",
        "avg_price",
        "payment_value",
        "payment_installments",
    ]:
        if f in rec and not isinstance(rec[f], (int, float)):
            errors[f] = "must be a number"

    return errors
FEATURE_COLS = REQUIRED_FIELDS  # or define separately if you prefer

def predict_batch_internal(records):
    df = pd.DataFrame(records)
    df = df[FEATURE_COLS]  # ensure correct order/columns
    proba = model.predict_proba(df)[:, 1]
    preds = (proba >= 0.5).astype(int)
    labels = np.where(preds == 1, "positive", "negative")
    return preds, proba, labels

@app.route("/predict", methods=["POST"])
def predict_single():
    try:
        payload = request.get_json()
        if not isinstance(payload, dict):
            return jsonify({"error": "Invalid input, expected JSON object"}), 400

        errors = validate_record(payload)
        if errors:
            return jsonify({"error": "Invalid input", "details": errors}), 400

        preds, proba, labels = predict_batch_internal([payload])

        return jsonify({
            "prediction": int(preds[0]),
            "probability": float(proba[0]),
            "label": labels[0],
        })
    except Exception as e:
        # TEMP for debugging: show actual error
        return jsonify({"error": "Internal server error", "details": str(e)}), 500

@app.route("/predict/batch", methods=["POST"])
def predict_batch_endpoint():
    try:
        payload = request.get_json()
        if not isinstance(payload, list) or len(payload) == 0:
            return jsonify({"error": "Invalid input, expected non-empty JSON array"}), 400
        if len(payload) > 100:
            return jsonify({"error": "Too many records (max 100)"}), 400

        all_errors = [validate_record(r) for r in payload]
        if any(all_errors):
            return jsonify({"error": "Invalid input", "details": all_errors}), 400

        preds, proba, labels = predict_batch_internal(payload)
        results = [
            {"prediction": int(p), "probability": float(pr), "label": lab}
            for p, pr, lab in zip(preds, proba, labels)
        ]
        return jsonify({"results": results})
    except Exception as e:
        return jsonify({"error": "Internal server error", "details": str(e)}), 500

if __name__ == "__main__":
    app.run(debug=True)